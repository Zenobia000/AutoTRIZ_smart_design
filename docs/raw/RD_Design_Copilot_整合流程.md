...
| Gate | 位置 | Phase 轉換 |
|------|------|-----------|
| **Gate 1.1** | Step 1.1 完成 | **DRAFT → PHASE_1** |
| Gate 1.2 | Step 1.2 完成 | Phase 1 內部 |
| **Phase Gate 1 (= Gate 1.3)** | Step 1.3 完成 | **PHASE_1 → PHASE_2** |
| Gate 2.1 | Step 2.1 完成 | Phase 2 內部 |
| **Phase Gate 2 (= Gate 2.3)** | Step 2.2 完成 | **PHASE_2 → PHASE_3** |
| Gate 3.1 | Step 3.1 完成 | Phase 3 內部 |
| Gate 3.2 | Step 3.2 完成 | Phase 3 內部 |
| **Phase Gate 3 (= Gate 3.3)** | Step 3.3 完成 | **PHASE_3 → COMPLETED** |

---

## 流程總覽（8 步驟 × 六帽 × SCAMPER/TRIZ）

> **v1.1 正名**：Step 編號按實際執行順序重新編排。原 Step 6(創造與調整)→**Step 2.2**，原 Step 5(SWOT)→**Step 3.1**。
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  Phase 1: Define — 定義問題空間 (Week 1-2)                               │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                  │
...
│                                        └─────────────┘                  │
│                                                                          │
│  Phase 2: Diverge — 假設與發散 (Week 2-4)                                │
│  ┌─────────────┐    ┌─────────────────────────────────────┐              │
│  │ Step 2.1    │ →  │ Step 2.2: 創造與調整 (綠帽/紅帽)   │              │
│  │ 假設驗證    │    │                                      │              │
│  │ (HDA)       │    │  矛盾句 → 2.2.2 TRIZ 解矛盾        │              │
│  │ +未知集合   │    │              ↓                       │              │
│  │             │    │         2.2.3 子系統定義              │              │
│  └─────────────┘    │              ↓                       │              │
│                      │         2.2.4 SCAMPER 模組變形       │              │
│                      │              ↓                       │              │
│                      │    TRIZ ──→ 2.2.5 AI 方案生成 ←── SCAMPER │       │
│                      │              ↓                       │              │
│                      │         2.2.6 MUST 快篩              │              │
│                      │           ↓       ↓                 │              │
│                      │         Pass    Fail(淘汰)          │              │
...
│                      └─────────────────────────────────────┘              │
│                                                                          │
│  Phase 3: Converge — 收斂與驗證 (Week 4-8)                               │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                  │
...
│           ↓ (每個子系統)                                         │
│  ┌──────────────────┐                                           │
│  │ 2.2.4 SCAMPER 變形│ ← 對每個子系統 × 7 動作                  │
│  │ 輸出: 七欄規格   │                                           │
│  └────────┬─────────┘                                           │
│           ↓                                                      │
│  ┌──────────────────┐                                           │
│  │ 2.2.5 AI 方案生成│ ← 整合 TRIZ + SCAMPER                     │
│  │ 附帶: 機制+假設  │                                           │
│  │    +風險+robust  │                                           │
│  │    +最小驗證     │                                           │
│  └────────┬─────────┘                                           │
...
**Phase 1: Define (Week 1-2): 把混沌變結構**
- [ ] 需求/限制模板完成
- [ ] 問題樹 + 矛盾列表 v0
- [ ] 假設台帳 v0
- [ ] 方案集合 v0（至少 3 條）
...
**Phase 2: Diverge (Week 3-6): 把結構變證據**
- [ ] 完成 Top 3 最小實驗
...
**Phase 3: Converge (Week 7-8): 把證據變資產**
...
| 步驟 | 核心產出 | Gate 條件 |
|------|---------|-----------|
| 1.1 | 任務定義表、約束清單 | Gate 1.1: 三個最不能失敗指標被定義 |
| 1.2 | 問題樹、矛盾列表 | Gate 1.2: 10 條假設 + Top 3 致命假設 + 3 條矛盾 |
| 1.3 | 因果迴路圖、TRIZ 矛盾句、斷路點 | Phase Gate 1: ≥1 因果迴路 + 3 斷路點 + TRIZ 正式句 |
| 2.1 | 假設台帳、未知集合 (U) | Gate 2.1: Top 3 假設有驗證設計 + ≥3 未知因子 |
| **2.2** | **TRIZ 解法→子系統→SCAMPER→方案集合→MUST 快篩** | **Phase Gate 2: 3+ 架構級路線 MUST 通過 + 完整方案規格** |
| **3.1** | **SWOT、黑帽質疑、風險登錄表** | **Gate 3.1: 風險有 Owner + 緩解 + 監控** |
| 3.2 | KT 決策記錄、最小實驗 | Gate 3.2: WANT 有證據 + H 風險有緩解 + 已簽核 |
| 3.3 | 一頁式、FAQ | Phase Gate 3: 跨層級可溝通 |
