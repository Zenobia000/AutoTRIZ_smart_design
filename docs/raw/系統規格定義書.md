...
> **v1.1 更新**：Step 編號按實際執行順序重新編排（原 Step 6→Step 5、原 Step 5→Step 6）；新增 Step 2.2 內部流程（TRIZ→子系統→SCAMPER→方案→MUST）詳細說明。
...
| phase | String(20) | DRAFT → PHASE_1 → PHASE_2 → PHASE_3 → COMPLETED |
...
```
                 ...
                 (Step 1.1→1.2→1.3)           (Step 2.1→2.2)         (Step 3.1→3.2→3.3)
                       │                          │                       │
DRAFT ──Gate 1.1──▶ PHASE_1 ──Phase Gate 1──▶ PHASE_2 ──Phase Gate 2──▶ PHASE_3 ──Phase Gate 3──▶ COMPLETED
                       │                          │                       │
                   Gate 1.2 (Step內)           Gate 2.1 (Step內)       Gate 3.1,3.2 (Step內)
```

| Gate | 位置 | Phase 轉換？ |
|------|------|-------------|
| Gate 1.1 | Step 1.1 完成後 | DRAFT → PHASE_1 |
| Gate 1.2 | Step 1.2 完成後 | — (Phase 1 內部) |
| Phase Gate 1 (= Gate 1.3) | Step 1.3 完成後 | **PHASE_1 → PHASE_2** |
| Gate 2.1 | Step 2.1 完成後 | — (Phase 2 內部) |
| Phase Gate 2 (= Gate 2.3) | Step 2.2 完成後 | **PHASE_2 → PHASE_3** |
| Gate 3.1 | Step 3.1 完成後 | — (Phase 3 內部) |
| Gate 3.2 | Step 3.2 完成後 | — (Phase 3 內部) |
| Phase Gate 3 (= Gate 3.3) | Step 3.3 完成後 | **PHASE_3 → COMPLETED** |
...
| 1.1 | 問題界定 | 1 | 任務定義表 |
| 1.2 | 理解全貌 | 1 | 索克拉底問答 |
| 1.3 | 系統建模 | 1 | 因果迴路 + TRIZ 矛盾 + 斷路點 |
| 2.1 | 假設與驗證規劃 | 2 | 假設台帳 + 未知集合 (U) |
| **2.2** | **創造與調整** | **2** | **TRIZ 解矛盾 → 子系統定義 → SCAMPER 變形 → AI 方案生成 → MUST 快篩** |
| **3.1** | **全方位檢視** | **3** | **SWOT + 黑帽質疑 + 風險登錄** |
| 3.2 | 決策與行動 | 3 | KT Decision Analysis + 最小實驗 |
| 3.3 | 內化與傳達 | 3 | 匯出報告 + 費曼 |
...
### Step 1.3 — 系統建模（因果迴路 + TRIZ 矛盾正式化 + 斷路點）

Step 1.3 包含三個子產出：**因果迴路圖**、**TRIZ 矛盾句**、**斷路點識別**。

---

#### Step 1.3a — 因果迴路圖 + 斷路點（AI 可建模）
...
| 使用者 | owner | 責任人 |
| 使用者 | due_date | 期限 (e.g. Gate 1.2 前) |

##### AI 協助

無直接 AI 生成。假設由 RD 工程師根據 Step 1.2 索克拉底問答與工程經驗填入。
...
- **導引 Robust 評判**：Step 3.2 WANT 條件中「餘裕深度」「公差鈍感」「解耦程度」的本質是方案在 U 各種組合下是否崩潰
...
Step 2.2 包含五個子步驟（2.2.2 ~ 2.2.6），形成串接式管線。
...
| 來源 | 欄位 | 說明 |
|------|------|------|
| 系統 | contradiction_id | 對應的矛盾 (來自 Step 1.3b) |
| 系統 | breakpoints | 相關斷路點 (來自 Step 1.3c)，提供介入位置提示 |
...
| 來源 | 說明 |
|------|------|
| Step 2.2.2 TRIZ 解法 | 工程對映中提到的子系統 |
...
確定的子系統清單，每個子系統會進入 Step 2.2.4 做 SCAMPER 變形。

---

#### Step 2.2.4 — SCAMPER 模組變形

##### 輸入

| 來源 | 欄位 | 說明 |
|------|------|------|
| Step 2.2.3 | subsystem | 子系統名稱 |
| Step 2.2.2 | TRIZ 解法方向 | 作為 SCAMPER 變形的參考方向 |
...
#### Step 2.2.5 — AI 方案生成（Set-Based 方案集合）

##### 目的

整合 Step 2.2.2 (TRIZ 解法) + Step 2.2.4 (SCAMPER 變形)，交叉組合生成完整的候選方案。

##### 輸入

| 來源 | 說明 |
|------|------|
| Step 2.2.2 | TRIZ 解法方向（原理+工程對映） |
| Step 2.2.4 | SCAMPER 變形候選（七欄輸出） |
| Step 2.1a | 假設台帳（方案需引用的假設） |
| Step 2.1b | 未知集合（方案需考量的不確定因子） |
...
用 MUST 條件做「快速淘汰」，不通過 = 直接淘汰。完整的 WANT 評分在 Step 3.2 執行。
...
4. 完整 KT Decision Analysis（MUST+WANT+AC）在 Step 3.2 執行
...
| 項目 | AI 可做 | AI 不可做 |
|------|---------|----------|
| Step 1.1 任務定義 | 建議 critical_metrics | 決定硬約束值 |
| Step 1.2 索克拉底 | 生成六類問題 | 替使用者回答 |
| Step 1.3 因果迴路 | 從問答+矛盾中自動建模因果迴路 + 識別斷路點 | 直接確認迴路正確性（人工審核） |
| Step 1.3 矛盾識別 | 從回答中初步識別矛盾 | 直接確認矛盾 |
| Step 2.2.2 TRIZ 解法 | 查表 + 生成工程對映 | 決定最終方案 |
| Step 2.2.4 SCAMPER | 生成七欄變形候選 | 省略失效模式/風險欄 |
| Step 2.2.5 方案生成 | 組合 TRIZ+SCAMPER 產出候選 | 使用形容詞式描述 |
| Step 2.2.6 MUST 評分 | — | **不參與** (人工 Go/No-Go) |
| Step 3.1 風險評估 | 黑帽審查產出風險候選 | 決定緩解措施 |
| Step 3.2a WANT 評分 | — | **不參與** (人工基於證據評分) |
| Step 3.2c 決策建議 | 彙整 MUST/WANT/AC 結果 | 決定首選方案 |
...
| Gate | 位置 | Phase 轉換 | 條件 |
|------|------|-----------|------|
| **Gate 1.1** | Step 1.1 → Step 1.2 | **DRAFT → PHASE_1** | critical_metrics ≥ 3 且各有 target+method |
| Gate 1.2 | Step 1.2 → Step 1.3 | — | contradictions ≥ 3, answered questions ≥ 10 |
| **Phase Gate 1 (= Gate 1.3)** | Step 1.3 → Step 2.1 | **PHASE_1 → PHASE_2** | causal_loops ≥ 1, breakpoints ≥ 3, 每條矛盾有 TRIZ 正式句 |
| Gate 2.1 | Step 2.1 → Step 2.2 | — | assumptions ≥ 10, Top 3 High 有驗證設計, unknown_factors ≥ 3 |
| **Phase Gate 2 (= Gate 2.3)** | Step 2.2 → Step 3.1 | **PHASE_2 → PHASE_3** | alternatives ≥ 3, MUST pass ≥ 3, 每個方案有完整規格 |
| Gate 3.1 | Step 3.1 → Step 3.2 | — | 每個風險有 Owner + mitigation + monitor_metric |
| Gate 3.2 | Step 3.2 → Step 3.3 | — | DecisionRecord 已簽核, H 風險有緩解, WANT 有證據 |
| **Phase Gate 3 (= Gate 3.3)** | Step 3.3 → Done | **PHASE_3 → COMPLETED** | 報告已產出 |
...
| 檔案 | 用途 | 對應 Step |
|------|------|-----------|
| task_definition.md | 生成任務定義建議 | Step 1.1 |
| socratic_questions.md | 生成索克拉底問題 | Step 1.2 |
| causal_loop_generate.md | AI 建模因果迴路 + 斷路點 | Step 1.3a |
| contradiction_identify.md | 識別矛盾 | Step 1.3b |
| triz_solution.md | TRIZ 解矛盾 | Step 2.2.2 |
| scamper_variant.md | SCAMPER 變形 | Step 2.2.4 |
| alternative_generate.md | 生成方案 | Step 2.2.5 |
| black_hat_review.md | 黑帽審查 | Step 3.1 |
| decision_record.md | 決策建議 | Step 3.2c |
